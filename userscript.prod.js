...
if (window.top !== window.self) {
  ...
  window.addEventListener('load', function() {
    loadOverlay()
    setInterval(loadOverlay, 5 * 60 * 1000) // 5 Minute reload timer
  ...
  function loadOverlay() {
    ...
    const link = "https://raw.githubusercontent.com/Crabbey/cool-s-treaty-rplace/master/cool-s-alliance-overlay.png?tstamp=" + time;
  ...
  function finishedLoad(){
    for (var x = 182; x <= 403; x++) {
      for (var y = 888; y <= 999; y++) {
        flashPixel(x, y)
      }
    }
  }
